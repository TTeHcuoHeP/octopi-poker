// Adapted from Aceternity UI 3D Globe: https://ui.aceternity.com/components/3d-globe
  ...
  return (
    <group ref={groupRef} visible={isVisible}>
      ...
      <mesh position={surfacePosition} quaternion={lineQuaternion}>
        ...
        <meshBasicMaterial color={hovered ? "#EEEFF1" : "#6C5CE7"} />
      </mesh>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            ...
            style={{
              width: marker.size ?? defaultSize,
              height: marker.size ?? defaultSize,
            }}
...
function Scene({ markers, config, onMarkerClick, onMarkerHover }: SceneProps) {
  ...
  React.useEffect(() => {
    const distance = config.radius * 3.5;
    camera.position.set(distance * Math.sin(config.initialRotation.y) * Math.cos(config.initialRotation.x), distance * Math.sin(config.initialRotation.x), distance * Math.cos(config.initialRotation.y) * Math.cos(config.initialRotation.x));
    camera.lookAt(0, 0, 0);
...
export function Globe3D({
  ...
  return (
    <div className={cn("globe-3d", className)}>
      <Canvas
